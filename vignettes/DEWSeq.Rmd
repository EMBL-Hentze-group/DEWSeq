...
and benchmarking, as well as feedback to the vignette. Mike Love, Simon Anders, 
...
Because of the properties of eCLIP or iCLIP data described above, we use a 
sliding window approach (where different window and step sizes can be chosen in 
the preprocessing) and test for significant enrichment in the IP over the
control. For this, we extended `r Biocpkg("DESeq2")` for a one-sided wald 
test to filter windows with negative wald stat values and do a right-tailed
test on the remaining windows. The filtering step increases the performance 
...
correction on binding site with the SIMES method. SIMES joins coverage blocks
...
overlapping window with the number of overlaps using Bonferroni. 
...
Then we apply multiple hypothesis correction on the corrected p-values.
...
* Second trimming step (extremely important for paired-end (standard) eCLIP
  ...
  protocol. Hint: Single-read seCLIP [@VanNostrand2017] has a lot of advantages 
  to standard,
