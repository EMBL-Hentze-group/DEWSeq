...
transcripts with different expression levels. UV crosslinking does not 
affect RNAs in a linear fashion, therefore an appropriate input control
is needed for the analysis of eCLIP/iCLIP data. Unfortunately, total
RNA-seq does not help, also IgG-, empty beads, or similar controls are 
not appropriate input controls. We recommend size-matched input controls
(SMI) controls as performed in the eCLIP protocol. This type of input 
control is not protocol specific, therefore can be easily adapted to
iCLIP or other CLIP studies. 
...
Please refer to the [csaw](http://bioconductor.org/packages/csaw) 
package for discussion on this topic.
...
interested in a transcriptome wide analysis of your RNA-binding protein.
