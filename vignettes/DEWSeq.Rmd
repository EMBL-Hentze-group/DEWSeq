...
understand processes sucha s RNA splicing, localisation, 
modification and degradation. Individual nucleotide resolution
crosslinking and immunoprecipitation (iCLIP) [@Konig2010] and 
...
transcribing the RNA, a majority of the time the reverse 
transcriptase will fall of at the crosslink site. Althrough
...
RNA modification sites or at crosslinking sites from other proteins. This this might be individual for many proteins (and the
...
load required packages for the rest of this vignette
...
affect nearby proteins. Therefore it is of great importance that crosslink 
sites are enriched compared to proper input controls.
...
digest RNA not protected by your protein. Your RNase and its concentration
will result in different length of the RNA fragment. Also, if your protein
is stacked tighly together protecting a larger area from digestion,
the crosslinking pattern can be affected as well [@Haberman2017]. 

```{r digestion pattern image, fig.cap = "Different RNase concentrations will result in different fragment sizes.", fig.small=TRUE, echo = FALSE} 
knitr::include_graphics("digestionpatterns.png")
```

#### Read-throughs

iCLIP and eCLIP rely that reverse transcriptases truncate on crosslink sites. 
HITS-CLIP and PAR-CLIP rely on reverse transcriptases reading through the 
crosslink sites. Both methods use different reverse transcriptases with 
different likelihood of truncating or reading through. A parameter for this
might also be the polypeptide which is left after protein digestion which
is different from protein to protein.

```{r readthrough image, fig.cap = "Read-throughs", fig.small = T, echo = FALSE} 
knitr::include_graphics("readthrough.png")
```

"Read-throughs" can truncate at a crosslink site from the same proteins,
a different protein, or stall at the end of the RNA fragment. Due to the 
low crosslinking efficency the crosslinking of multiple proteins is usually
less likely. 
...
efficency should make this less likely to have multiple crosslink sites
at once, those events might occur. If more early truncation events occur, 
the binding of 
...
quality control plots like MA plots out. Please find very detailed explanation 
