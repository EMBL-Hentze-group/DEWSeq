...
#' @return data.frame
createRegions <- function(mergeDat,
                          padjCol              = 'pBonferroni.adj',
                          padjThresh           = 0.05,
                          log2FoldChangeCol    = 'log2FoldChange',
                          log2FoldChangeThresh = 1,
                          ncores = 5) {
  requiredCols <- c('unique_id',
                    'chromosome',
                    'begin',
                    'end',
                    'strand',
  ...
  missingCols <- setdiff(requiredCols,
                         colnames(mergeDat))
  ...
  regionDat <- do.call(rbind,
                       bplapply(geneList,
                                createRegionsHelper,
                                padjCol              = padjCol,
                                padjThresh           = padjThresh,
                                log2FoldChangeCol    = log2FoldChangeCol,
